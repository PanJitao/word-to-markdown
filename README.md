...
它用 [Tauri v2](https://tauri.app/)（系统 WebView2）+ 原生 TypeScript 重写，**不再捆绑 Chromium 或 Python 运行时**，相比传统 Electron 方案体积缩小约 **45 倍**（137 MB → 3 MB）。所有文档转换都在前端 JS 完成，启动即用、关闭即走。

| | 旧方案（Electron + Python） | **ExchangeMD（本项目）** |
| --- | --- | --- |
| 体积 | 137 MB | **~3 MB** |
| 运行时 | 内置 Chromium + Python | 系统 WebView2 + 纯前端 |
| 后端进程 | 常驻 Python 服务 | 无 |
| 安装 | 安装包 | 单文件 exe，免安装 |
